...
class OpenWiki {
		...
		*/
		...
		if ($this->isLoaded() ) {
			$sql = "UPDATE openwiki SET 
				...
				descr ='" . $this->getDescr() . "', 
				owner = '" . $this->getOwner() . "', 
				access = " . $this->getAccess() . " WHERE id = '" . $this->getIdentifier() . "'";
			...
			$res = mysql_query("INSERT INTO openwiki (id, name, descr, owner, access) values ('" . 
				$this->getIdentifier() . "','" . $this->getName() . "', '" . $this->getDescr() . "', '" . 
				$this->getOwner() . "', " . $this->getAccess() . ")", $this->db);
			if(mysql_error()){
				throw new Exception('Invalid query: ' . mysql_error());
			}
		}
	...
	private function deleteACLinDB() {
		...
		$res = mysql_query("DELETE FROM acl WHERE wikiid='" . $this->getIdentifier() . "'", $this->db);
		if(mysql_error()){
			throw new Exception('Invalid query: ' . mysql_error());
		}
	}
		...
		*/
		...
		foreach ($this->customacl AS $priority => $entry) {
			...
			$res = mysql_query("INSERT INTO acl (wikiid, name, access, priority) values ('" . 
				$this->getIdentifier() . "','" . $entry[0] . "', " . $entry[1] . ", " . $priority . ")", $this->db);
			if(mysql_error()){
				throw new Exception('Invalid query: ' . mysql_error());
			}
	...
	public function getACLdefinition() {
		...
		$def .= $this->getIdentifier() . ':* ' . $this->auth_nameencode($this->getOwner()) . ' 15' . "\r\n";
		$def .= "\r\n\r\n";
		return $def;
	}
